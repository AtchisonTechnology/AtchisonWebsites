...
| Spec0017 | Make the leeatchison.com home page Books section collection-driven | Replace the four hardcoded `.book-card` blocks in `index.erb` with a loop over the shared books collection, fixing the card order that disagreed with `order_leeatchison` (1, 3, 2, 4), the hand-written BB3.0 author line and the forked Software Conductor blurb, and adding the `/books` link the section never had — a card links both to the local book page and off-site (`book_url` then `amazon_url`) — [PR #22](https://github.com/AtchisonTechnology/AtchisonWebsites/pull/22) | Closed | 2026-09-01 |
| Spec0018 | Center the featured-books grid when fewer than four books are featured | Replace `.books-grid`'s `repeat(4, 1fr)` with `repeat(auto-fit, minmax(220px, 240px))` plus `justify-content: center` in both stylesheets, fixing the left-hugging two-card row on atchisonacademy.com/books; delete the Academy home page's duplicate override and the two responsive column overrides that specificity had always beaten — the spec's original 260px cap was corrected to 240px during implementation after testing showed it would have broken leeatchison.com's four-across row — [PR #23](https://github.com/AtchisonTechnology/AtchisonWebsites/pull/23) | Closed | 2026-09-01 |
